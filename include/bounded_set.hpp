...
namespace IVMPG {
  namespace Container {
    ...
    class bounded_set {
      ...
      // This is just a copy of the previous unique_ptr which allows to gain a
      // few percents speed using direct access.
      Pair* buckets;
      Pair* first;
#ifdef SET_STATISTIC
     ...
     public:
      ...
      // We use an extra uninitialized pair as a sentinel for the end of the
      // linked list.
      bounded_set()
          : buckets_own(new Pair[bound + 1]),
            buckets(buckets_own.get()),
            first(&(buckets[bound])) {
    ...
    // template <class Key, class Hash, size_t bound>
    // auto bounded_set<Key, Hash, bound>::operator=(bounded_set&& rhs) &
    // noexcept -> bounded_set & {
    //   assert(this != &rhs);
    ...
    std::ostream& operator<<(std::ostream&                        stream,
                             bounded_set<Key, Hash, bound> const& set) {
      bool first = true;
      stream << "{";
      for (auto x : set) {
        if (not first)
